...
/**	
  ...
  *	In this file, we shall call this data structure the 'Sudoku Box'.
*/
class SudokuBox{
	...
	*/
	...
	/**
	  *	@desc Displays a menu to the user when the SudokuBox objects in instantiated
	...
	*/
	private:void menu(){
		...
		cout<<"You can either:-\n";
		cout<<"\t1. Input the puzzle by entering the values manually. (Enter 1 to do so)\n";
		cout<<"\t2. Input the puzzle by reading a file with values in it. (Enter 2 to do so)\n";
		...
		if(option==1) readFrameValues();
			...
			while(true){
				cout<<"\nYou seem to have entered an invalid option. Please try again!\n";
				cout<<"\t   --> ";
				cin>>option;
			...
			for(colIter=0; colIter<9; colIter++){
				...
				if(!(enteredValue>=0 && enteredValue<=9)){ //Checking for bounds in input.
					while(true){ //We loop until valid input is read from user.
						cout<<"Oops! You seem to have entered a wrong value! Please try again.\n";
						cout<<"Enter value for cell ["<<rowIter+1<<"]["<<colIter+1<<"] --> ";
						cin>>enteredValue;
	...
	private:void readFrameValuesFile(){
		...
		for(rowIter=0; rowIter<9; rowIter++){ //Iterating over file values.
			for(colIter=0; colIter<9; colIter++){
				...
				sudokuFile>>readValue; //Reading the values from the file
...
class SudokuSolver{
	...
	SudokuBox frame; //The frame object
